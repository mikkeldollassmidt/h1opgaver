...
namespace Mozart
{
    internal class Program
    {
        ...
        {
            // Creating array¨s for all our information.
            ...
            // string array called files with room for "menuets.GetLength(1) * 2", which is 32. Take the first number in each collumn and multiply with 2.
            string[] files = new string[menuets.GetLength(1) * 2];
    
            // Creating an object.
            Random random = new Random();

            // Making integers for throwing dice and calculating them to choose from our arrays in the top.
            int throwDice = 0;
            int throwIndex = 0;

            Console.WriteLine("Start playing music with the ENTER button.");
            Console.ReadLine();

            // Just to jump in and out of while loop
            bool whileLoopRun = true;
            
            // While loop to keep the program organized in a loop.
            while (whileLoopRun)
            {
                // A for to fill up the array files with the dice chosen music file. (For menuets)
                for (int i = 0; i < menuets.GetLength(1); i++)
                {
                    throwDice = random.Next(0, 5);
                    throwIndex = throwDice;
                    throwDice = random.Next(0, 5);
                    throwIndex += throwDice;

                    files[i] = @"C:\Users\Dollas\Desktop\PROGRAMMERING\Mozart\wave\M" + menuets[throwIndex, i] + ".wav";

                    throwIndex = 0;
                }
                // A for to fill up the array files with the dice chosen music file. (For trios)
                for (int i = 0; i < trios.GetLength(1); i++)
                {
                    throwDice = random.Next(0, 6);
                    throwIndex = throwDice;

                    files[i] = @"C:\Users\Dollas\Desktop\PROGRAMMERING\Mozart\wave\T" + trios[throwIndex, i] + ".wav";
